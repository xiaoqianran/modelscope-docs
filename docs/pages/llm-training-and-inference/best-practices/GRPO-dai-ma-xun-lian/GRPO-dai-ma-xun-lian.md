<!-- modelscope-docs: GRPO Code Training | llm-training-and-inference/best-practices/GRPO-dai-ma-xun-lian/GRPO-dai-ma-xun-lian_EN.md -->

---
url: code-grpo
---
# GRPO Code Training
This document describes how to use GRPO for code training on models.
...
`verification_info` provides the programming language and test cases, including input and expected output.


## Reward Function
Training is performed using `code_reward` and `code_format` rewards. Implementation details can be found in the [code](https://github.com/modelscope/ms-swift/blob/main/examples/train/grpo/plugin/plugin.py).

- `code_reward` executes the generated code through [e2b](https://e2b.dev/) or [judge0](https://judge0.com/) and validates the code against test cases provided in the dataset to give a reward value.
- `code_format` requires the model to output formatted responses containing code blocks.

Note: Currently, code execution via e2b only supports Python language. For executing other languages, you can use judge0 ([list of languages supported by judge0](https://github.com/judge0/judge0?tab=readme-ov-file#supported-languages)).

## Training Scripts
### e2b
- Register at [e2b](https://e2b.dev/dashboard) to obtain E2B_API_KEY and set it as an environment variable.
- Add `external_code_reward` as a reward function in `--reward_funcs`.
...
- Set environment variables:
    - (Required) JUDGE0_ENDPOINT: judge0 access endpoint.
    - (Optional) JUDGE0_X_AUTH_TOKEN: judge0 access token.
- Add `external_code_reward_by_judge0` as a reward function in `--reward_funcs`.
- Set `--external_plugins` to the path of plugin.py.
...
Training reward curve chart
![Training reward curve chart](./_resources/grpo_code.png)
<!-- modelscope-docs: NPU Support | llm-training-and-inference/best-practices/npu/npu_EN.md -->

# NPU Support

We have added support for Ascend NPU in ms-swift, allowing users to perform model fine-tuning and inference on Ascend NPU.

This document introduces how to prepare the environment, fine-tune models, perform inference, and deployment on Ascend NPU.
...
For basic environment preparation, please refer to this [Ascend PyTorch Installation Documentation](https://gitcode.com/Ascend/pytorch).

## Environment Preparation

Experimental environment: 8 * Ascend 910B3 64G
...
# Note: Before proceeding with subsequent operations, you need to source to activate the CANN environment
source /usr/local/Ascend/ascend-toolkit/set_env.sh

# Set pip global mirror (optional, to accelerate downloads)
...
# If you want to use deepspeed (to control VRAM usage, training speed will decrease somewhat)
pip install deepspeed

# If you need to use evaluation functionality, please install the following packages
...
Test whether the environment is installed correctly and whether NPU can be loaded properly:
```python
...
**If you need to use MindSpeed (Megatron-LM), please follow the guide below to install necessary dependencies**
```shell
# 1. Get and switch Megatron-LM to core_v0.12.1 version
git clone https://github.com/NVIDIA/Megatron-LM.git
cd Megatron-LM
git checkout core_v0.12.1
cd ..

# 2. Get and install MindSpeed
...
Execute the following command to verify whether MindSpeed (Megatron-LM) is configured successfully:
```shell
python -c "import mindspeed.megatron_adaptor; from swift.megatron.init import init_megatron_env; init_megatron_env(); print('✓ Megatron-SWIFT configuration verification successful in NPU environment!')"
```

### Environment Inspection

Check NPU P2P connections. Here we can see that each NPU is interconnected with other NPUs through 7 HCCS links:
...
Check NPU status. For detailed npu-smi command documentation, please refer to the [official documentation](https://support.huawei.com/enterprise/zh/doc/EDOC1100079287/10dcd668):
...
The following introduces LoRA fine-tuning. For full-parameter fine-tuning, simply set the parameter `--train_type full`. **More training scripts** can be found [here](https://github.com/modelscope/ms-swift/tree/main/examples/ascend/train).

| Model Size | NPU Count | Deepspeed Type | Maximum VRAM Usage |
| ---------- | --------- | -------------- | ------------------ |
| 7B         | 1         | None           | 1 * 28 GB          |
| 7B         | 4         | None           | 4 * 22 GB          |
| 7B         | 4         | zero2          | 4 * 28 GB          |
...
### Using Native PyTorch for Deployment
...
### Using vLLM-ascend for Deployment
...
### Table 3: Modules Currently Not Supported / Not Fully Verified on NPU

| item                   |
| ---------------------- |
| Liger-kernel           |
| Quantization/QLoRA related |

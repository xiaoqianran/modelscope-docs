<!-- modelscope-docs: Custom Model Evaluation | model-evaluation/advanced-guides/custom-model/custom-model_EN.md -->

EvalScope supports evaluation of various models based on the `ModelAPI` abstract interface. This article will introduce how to implement a custom model adapter by referencing `MockLLM`.

## When Do You Need a Custom Model Adapter?

You may need to create a custom model adapter in the following situations:

- Your model doesn't support the standard OpenAI API format
- You need special processing for model inputs and outputs
- You need to use specific inference parameters or configurations

## ModelAPI Interface Definition

All models need to implement the `ModelAPI` abstract base class:
...
class MyCustomModel(ModelAPI):
    ...
    def _call_model(self, input_text: str, config: GenerateConfig) -> str:
        ...
        # For example: call API, local model inference, etc.
...
### Usage via Registration

```python
from evalscope import run_task, TaskConfig
from xxx import MyCustomModel # Need to import the model beforehand to enable auto-registration
...
2. **Return `ModelOutput`** objects, which can be created using `ModelOutput.from_content()`
3. **Handle exceptions** to ensure model call stability
4. **Register models** for use in configurations
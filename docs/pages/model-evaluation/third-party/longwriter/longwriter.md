<!-- modelscope-docs: LongBench-Write | model-evaluation/third-party/longwriter/longwriter_EN.md -->

# LongBench-Write

## Description
LongWriter enables LLMs to generate content exceeding 10,000 words. We can use the benchmark **LongBench-Write** to evaluate the quality of long outputs and the ability to generate text of the required length on demand.
...
Supported task configuration formats: dict, json, yaml.

1. Dict configuration:
...
- Arguments:
  ...
  - `model`: model id on the ModelScope hub, or local model dir. Refer to [LongWriter-glm4-9b](https://modelscope.ai/models/ZhipuAI/LongWriter-glm4-9b/summary) for more details.
...
### Launch Model Inference Service
It is recommended to use [vLLM](https://github.com/vllm-project/vllm) to deploy the model service.
...
* Tip: You can use `CUDA_VISIBLE_DEVICES=0,1,2,3` to configure a multi-GPU environment.

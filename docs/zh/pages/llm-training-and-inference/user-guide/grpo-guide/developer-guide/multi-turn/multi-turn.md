<!-- modelscope-docs: 多轮训练 | llm-training-and-inference/user-guide/grpo-guide/developer-guide/multi-turn/multi-turn_CN.md -->
...
![多轮示例图](./_resources/grpo_multi_turn.png)
...
ms-swift 提供 loss_scale 参数来对模型回复部分的内容进行损失缩放设置。比如设置`--loss_scale last_round`，可以将非最后一轮的模型回复的损失置零。我们也可以实现自定义 loss_scale，具体请参考[定制化 loss_scale 文档](../../../定制化/插件化.md#定制化loss_scale)。
...
### 训推一致性兼容
swift >= 3.11 支持从 vLLM 侧返回 rollouot 的 logps 用于纠正训推不一致问题，具体请参考该[文档](../先进算法/training_inference_mismatch.md)

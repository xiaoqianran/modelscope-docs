<!-- modelscope-docs: 预训练与微调 | llm-training-and-inference/user-guide/pretraining-and-finetuning/pretraining-and-finetuning_CN.md -->
...
## 环境准备
推荐的第三方库版本参考[SWIFT安装文档](../入门介绍/SWIFT安装.md)
```bash
...
## 预训练
预训练使用`swift pt`命令，这将自动使用生成式而非对话式的template，即将`use_chat_template`设置为False（其他所有的命令，例如`swift sft/rlhf/infer`，都默认将`use_chat_template`设置为True）。此外，`swift pt`与`swift sft`相比，具有不同的数据集格式，可以参考[自定义数据集文档](../定制化/自定义数据集.md)。
...
我们提供了10分钟在单卡3090上对Qwen2.5-7B-Instruct进行自我认知微调的最佳实践，具体参考[这里](../入门介绍/快速开始.md)，这可以帮助您快速了解SWIFT。
...
- 多模态训练：SWIFT支持多模态模型的预训练、微调和RLHF。支持Caption、VQA、OCR、[Grounding](https://github.com/modelscope/ms-swift/blob/main/examples/notebook/qwen2_5-vl-grounding/zh.ipynb)任务。支持图像、视频和音频三种模态。具体参考[这里](https://github.com/modelscope/ms-swift/tree/main/examples/train/multimodal)。多模态自定义数据集格式参考[自定义数据集文档](../定制化/自定义数据集.md)。
...
### 使用Web-UI
如果你要使用界面的方式进行训练，可以查看[Web-UI文档](../入门介绍/Web-UI.md)。

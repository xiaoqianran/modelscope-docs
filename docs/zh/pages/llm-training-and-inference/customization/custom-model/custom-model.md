<!-- modelscope-docs: 自定义模型 | llm-training-and-inference/customization/custom-model/custom-model_CN.md -->
...
每种model_type都有唯一的模型结构、template和加载方式。当然，你也可以手动传入`--model_type`、`--template`来进行覆盖。ms-swift已支持的model_type和template可以查看[支持的模型与数据集](../功能指引/支持的模型和数据集.md)。

以下介绍如何注册一个新模型和对应的template。最佳实践参考[注册多模态模型最佳实践](../最佳实践/MLLM-Registration.md)。

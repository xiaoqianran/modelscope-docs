<!-- modelscope-docs: SCRFD_face_detection | model-overview/vision/SCRFD-face-detection/SCRFD-face-detection_CN.md -->

# 模型概览
SCRFD为当前SOTA的人脸检测方法，已被ICLR-2022接收([论文地址](https://arxiv.org/abs/2105.04714), [代码地址](https://github.com/deepinsight/insightface/tree/master/detection/scrfd))。SCRFD famlity在WIDERFace-Hard的指标如下：
![模型结构](./_resources/SCRFD-sota.jpg)
...
![对比](./_resources/compare.jpg)

## 技术特点
本模型提供一种高效且精准的人脸检测器，同时支持极小脸、极大脸以及任意角度的人脸检测，并且支持五点关键点的输出。
...
## 参数说明
- 输入：仅有一个检测分的阈值参数，位于配置文件`mmcv_scrfd.py`的`model.test_cfg.score_thr`,默认0.3
- 输出：按照如下格式输出检测分、矩形框和关键点坐标（其中矩形框坐标为左上角、右下角；关键点为左眼、右眼、鼻尖、左嘴角、右嘴角）
```
    {
...
face_detection = pipeline(Tasks.face_detection, 'damo/cv_resnet_facedetection_scrfd10gkps', model_revision='v2')  # set model_revision='master' for offical scrfd_10g_bnkps model
...
* **train**: 训练集相关的参数
  ...
  * pipeline: 训练数据按照所配置的模块依次做预处理的过程，之后输入模型进行训练
    ...
    * RandomSquareCrop: 随机裁减正方形区域
      ...
      * big_face_ratio: 从原图中选择一个面部区域替换为待裁剪原图的比例
      * big_face_crop_choice: 处于big_face模式时，随机选择的缩放比例，用于采样到超大脸
...
```
包括backbone, neck, head部分的网络结构信息，一般无需修改
## 训练示例代码
通过使用托管在modelscope DatasetHub上的数据集[WIDER_FACE](https://modelscope.cn/datasets/shaoxuan/WIDER_FACE)进行训练（简便起见，以下示例代码使用迷你数据集，并缩短了训练时长和batch_size，使用单卡做训练）：
```python
import os
import tempfile
...
- 本模型使用8卡v100，使用SGD优化器，lr=0.02，在440/544epoch时降低10倍学习率，并在640epoch时产出模型，数据增强方面相比原论文，增加了0.2概率的极大脸采样，以及0.3概率的图像旋转，其余超参与原论文一致

<!-- modelscope-docs: Pipeline默认配置 | sdk/tutorials/pipeline-default-config/pipeline-default-config_CN.md -->

# 推理任务的默认模型

本文档描述使用推理时不指定模型时使用的默认配置。如对应任务在下表中未找到，该任务可能无法在不指定模型的场景下运行。
